...
class PageListItems extends Component {
    constructor(props) {
      ...
      this.state = {
        topics: ["Lifestyle and Culture",  
          "Gender",
          "Maritime Economy",
          "Archaeology and History"
          ],
        subjectTags: ["Slavery","War/Military","Pearl Diving","Education","Marriage","Oil/Petroleum Industry","Transportation","Construction","Medical","Clothing","Food","Family","Majilis","Language","Boats/Ships","Fishing","Arabic Writing/Calligraphy","English Writing","Shops/Businesses","Natural Areas","Mosques","Museums","Religion","Music","Agriculture/Farms","Water","Housing"],
        locations: [
          "Al Dafna",
          "New Delma",
          "The Old Souq",
    ...
    if (document.getElementById(event.target.id).checked == true) {
      let changeTags = async () => { 
        if (event.target.className == "subjectTags") {
          newSubjectTagsArray = [...self.state.checkedSubjectTags, event.target.id];
            if (this.state.checkedSubjectTags.includes(event.target.id)) {
              newSubjectTagsArray = newSubjectTagsArray.filter(tag => tag !== event.target.id);
            }
          }
          ...
          if (this.state.checkedTopics.includes(event.target.id)) {
            newTopicsArray = newTopicsArray.filter(tag => tag !== event.target.id);
          }
        }
          ...
          if (this.state.checkedLocations.includes(event.target.id)) {
            newLocationsArray = newLocationsArray.filter(tag => tag !== event.target.id);
          }
        }
      }
    ...
    render() {
        ...
        <>
            ...
            <div className="dropdownFilter">
              ...
              <h2>Topics:</h2>
                {this.state.topics.map((info, index) => (
                  <div>
                    <input type="checkbox" class="topics" onChange={this.tagChange} id={info}></input>    
                    <span className="checkmark"></span>
                    <label className="checkboxContainer">{info}
                    </label>
                  </div>
                ))}
              <h2>Subject Tags:</h2>
                {this.state.subjectTags.map((info, index) => (
                  <div>
                    <input type="checkbox" class="subjectTags" onChange={this.tagChange} id={info}></input>    
                    <span className="checkmark"></span>
                    <label className="checkboxContainer">{info}
                    </label>
                  </div>
                ...
                {this.state.locations.map((info, index) => (
                  <div>
                    <input type="checkbox" class="locations" onChange={this.tagChange} id={info}></input>    
                    <span className="checkmark"></span>
                    <label className="checkboxContainer">{info}
                    </label>
                  </div>
            ...
            <div id="articleCardsWrapper">
              ...
              {this.state.locationData.map((info, index) => (
                  ...
                  <div key={index} className="articleCard">
                    ...
                    {/* <img src={BeachImage} /> */}
                    <img src={info.imagePaths[0] != "" ? require('../images/' + info.imagePaths[0]) : BeachImage} />
          ...
          <div id="showResultsNumberMobile">
              <button onClick={this.filterMobileList}><div>View {this.state.locationData.length} Results</div></button>
          </div>
        </>
        )
      }
