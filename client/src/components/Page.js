  ...
  render() {
    if (this.state.data_received == false) {
      ...
      const filteredData = this.state.rawData.filter(
        entry => entry.id == this.props.match.params.id
      );
      ...
      return (
        <div>
          ...
          <div id="pageContentWrapper">
            <h1>{filteredData[0].locationName}</h1>
            {filteredData[0].imagePaths.length >= 1 &&
            filteredData[0].imagePaths[0] != "" ? (
              <img
                src={require("../images/" + filteredData[0].imagePaths[0])}
                alt="Place Holder Beach"
                className="bigImagePage"
              />
            ) : (
              <img
                src={BeachImage}
            ...
            {filteredData[0].audioPaths.length >= 1 &&
            filteredData[0].audioPaths[0] != "" ? (
              <audio controls className="audio-custom">
                {" "}
                <source
                  src={require("../sound/" + filteredData[0].audioPaths[0])}
                  type="audio/mp4"
                ></source>
              </audio>
            ) : (
            ...
            {filteredData[0].audioPaths.length >= 2 ? (
              <audio controls className="audio-custom">
                {" "}
                <source
                  src={require("../sound/" + filteredData[0].audioPaths[1])}
                  type="audio/mp4"
                ></source>
              </audio>
            ) : (
            ...
            {filteredData[0].videoPaths.length > 1 ? (
              <div class="video-container">
                <iframe
                  frameborder="0"
                  src={filteredData[0].videoPaths[0]}
                  frameborder="0"
